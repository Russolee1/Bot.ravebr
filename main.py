...
moderators = ["iMooseMoo","beatrizy.montenegro","Thaay__","Russo_lee","Docinho06","belatrxx"]
...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")  
        await self.highrise.walk_to(Position(5.5, 13.0 , 1.0 , "FrontLeft"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala") 

        await self.highrise.chat(f"SEJA BEM-VINDO(A) A RAVE BAR, RESPEITEM AS REGRAS E SE DIVIRTAM MUITO EM NOSSA SALA!")
        await asyncio.sleep(1)
        await self.highrise.chat(f"¡BIENVENIDO(A) A RAVE BAR, RESPETE LAS REGLAS Y DIVIÉRTASE MUCHO EN NUESTRA SALA!")
        await asyncio.sleep(1)
        await self.highrise.chat(f"WELCOME TO RAVE BAR, RESPECT THE RULES AND HAVE A LOT OF FUN IN OUR ROOM!")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith(("pista","/pista","Pista","!pista")):
            await self.highrise.teleport(user.id, Position(2.5 , 0.35 , 11.0))
            
        if message.lower().startswith(("/adm","adm","!adm","Adm")):
          if user.username in moderators:
            await self.highrise.teleport(user.id, Position(13.0 , 6.5 , 3.5))
            
        if message.lower().startswith(("!piso1","/piso1","F1","!t1","piso1")):
            await self.highrise.teleport(user.id, Position(8.5 , 1.5 , 11.5))

        if message.lower().startswith(("!piso3","/piso3","F3","!t3","piso3")):
            await self.highrise.teleport(user.id, Position(11.5 , 10.5 , 20.5))
            
        if message.lower().startswith(("!piso2","/piso2","F2","!t2","piso2")):
            await self.highrise.teleport(user.id, Position(9.5 , 13.0 , 2.5))

        if message.lower().startswith(("DJ","/dj","Dj","!dj","dj")):
            await self.highrise.teleport(user.id, Position(7.0 , 4.75 , 1.5))
        ...
        if message.lower().lstrip().startswith(("!info","/info","-info","info")):
            await self.highrise.send_whisper(user.id, f"🤖Bot criado por @iMooseMoo antigo @Ashokk no ano de [2023]\n\ncompra de bots chame @iMooseMoo no privado.") 
...
Item(type='clothing', amount=1, id='bag-n_registrationavatars2023furrytail', account_bound=False, active_palette=-1),
    ...
    async def on_user_leave(self, user: User) -> None:
        ...
        await self.highrise.chat(f"🚨Se prescisar de alguma ajuda use !help na minha DM ou na SALA.")
